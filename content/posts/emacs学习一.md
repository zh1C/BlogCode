...
注释操作由[evil-nerd-commenter](https://github.com/redguardtoo/evil-nerd-commenter)package实现，常用有如下快捷键：

| 快捷键               | 描述               |
| -------------------- | ------------------ |
| `SPC ;`              | 开启注释操作       |
| `SPC c l`、`SPC c L` | 行注释与行解注释   |
| `SPC c h`            | 隐藏或显示所有注释 |

> 快速多行注释和解注释的spacemacs操作方式为: **`SPC ; SPC j l`**
...
>
> 搜索匹配后是高亮显示：可以输入`:noh`结束高亮或者使用快捷键`SPC s c`
...
永久设置字体大小可以在`dotspacemacs-default-font`中进行设置，如果需要临时调整当前buffer的字体大小，可以使用emacs对应快捷键
...
| 快捷键                                   | 描述                            |
| ---------------------------------------- | ------------------------------- |
| `SPC !`                                  | 在minibuffer中运行一条shell命令 |
| `SPC '`                                  | 打开shell buffer                |
| `C-c C-c`或者命令`term-interrupt-subjob` | 终止当前正在运行的shell命令     |
