...
>2. 无法处理“浮动垃圾”：在进行垃圾回收过程中，由于用户线程也在运行，可能也会产生垃圾，称之为浮动垃圾，本次GC无法处理。
>3. 由于是标记-清除算法，则会产生大量内存碎片。
...
> 双亲委派模型的好处：
>
> Java中的类随着它的类加载器一起具备了一种带有优先级的层级关系。例如：`java.lang.Object`类，存放在rt.jar中，无论哪一个类加载器都需要加载这个类，最终都会委派给处于模型顶端的启动类加载器进行加载，因此Object类在程序的各种类加载器环境中都能够保证是同一个类。**双亲委派模型保证了 Java 程序的稳定运行，可以避免类的重复加载，也保证了 Java 的核心 API 不被篡改。**
